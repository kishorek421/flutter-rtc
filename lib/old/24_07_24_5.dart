...
class _WebRTCPageState extends State<WebRTCPage> {
  ...
  void _createOffer() async {
    ...
    // await _peerConnection?.setLocalDescription(RTCSessionDescription(optimizedSdp, description.type));
    await _peerConnection?.setLocalDescription(description);
    // String sdp = json.encode({'sdp': description.sdp, 'type': description.type});
    String sdp = json.encode(description.toMap());
    Fluttertoast.showToast(msg: "sdp -> $sdp");
    log("sdp -> $sdp");
  ...
  void _createAnswer() async {
    ...
    // await _peerConnection!.setLocalDescription(RTCSessionDescription(optimizedSdp, description.type));
    await _peerConnection!.setLocalDescription(description);
    // String sdp = json.encode({'sdp': description, 'type': description.type});
    String sdp = json.encode(description.toMap());
  ...
  void _listenForSms() async {
    ...
    easySmsReceiver.listenIncomingSms(
      onNewMessage: (msg) {
        ...
        try {
          Map<String, dynamic> message = json.decode(receivedMessage);
